...
function maintenanceReportText(report) {
...
function renderMaintenanceItems(report) {
  if (!report) {
  ...
  return `
    <div class="ltracker-maintenance-list">
      ${report.items.map((item) => `
        <article class="ltracker-maintenance-item" data-severity="${escapeHtml2(item.severity)}">
          <div class="ltracker-command-card-header">
            <span class="ltracker-card-title">${escapeHtml2(item.category)}</span>
            <span class="ltracker-status-chip" data-tone="${item.severity === "error" ? "error" : item.severity === "repairable" ? "warning" : item.severity === "warning" ? "warning" : "success"}">${escapeHtml2(item.severity)}</span>
          </div>
          <div class="ltracker-card-body">${escapeHtml2(item.message)}</div>
          ${item.suggestedFix ? `<p class="ltracker-note">${escapeHtml2(item.suggestedFix)}</p>` : ""}
        </article>
      `).join("")}
    </div>
  `;
...
function normalizeDrawerPanel(value) {
  if (value === "home" || value === "presets" || value === "renderLab" || value === "display" || value === "more" || value === "generation" || value === "connection" || value === "memory" || value === "maintenance" || value === "diagnostics" || value === "advanced") {
    return value;
  }
  return null;
}
function drawerPanelPrimaryId(panel) {
  return panel === "generation" || panel === "connection" || panel === "memory" || panel === "maintenance" || panel === "diagnostics" || panel === "advanced" ? "more" : panel;
}
function setup(ctx) {
  const cleanups = [];
  ...
  function readSettings() {
    ...
    return {
      ...
      injection: {
        ...
        isolationMode: injectionSelectValue("isolationMode", state.settings.injection.isolationMode),
  ...
  function render() {
    ...
    const maintenanceTone = maintenanceReport?.status === "error" ? statusTone("error", "Error") : maintenanceReport?.status === "repairable" ? statusTone("warning", "Repairable") : maintenanceReport?.status === "warning" ? statusTone("warning", "Warning") : statusTone("success", maintenanceReport ? "Healthy" : "Ready");
    ...
    const commandCenterHtml = `
      ...
      return `<button class="ltracker-nav-chip" type="button" data-panel-target="${escapeHtml2(id)}" data-active="${active ? "true" : "false"}" aria-current="${active ? "page" : "false"}">${escapeHtml2(label)}</button>`;
        ...
        <section class="ltracker-panel ltracker-section" id="ltracker-section-memory-context">
          ...
          <div class="ltracker-card-grid">
            ${card("Tracker Memory", state.settings.memory.includeInTrackerGeneration ? statusTone("active", "Generator only") : statusTone("warning", "Stored only"), escapeHtml2("Used only when LTracker generates or updates tracker JSON."), "")}
            ${card(
      "Prompt Injection",
      ...
      escapeHtml2(`Injects tracker state into normal roleplay generation. Isolation: ${state.settings.injection.isolationMode.replace(/_/g, " ")}.`),
      `<button class="ltracker-button" type="button" data-action="copy-prompt-injection-safety-report" ${disabled(!diagnostics.lastPromptInjectionSafetyReport)}>Copy Safety Report</button>`
        ...
        <section class="ltracker-panel ltracker-section" id="ltracker-section-maintenance">
          ...
          <div class="ltracker-card-grid">
            ${card("Health check", maintenanceTone, escapeHtml2(maintenanceSummary), `<button class="ltracker-button" type="button" data-action="run-health-check">Run Health Check</button>`)}
            ${card("Snapshot index", statusTone((maintenanceReport?.duplicateIndexEntries ?? 0) + (maintenanceReport?.missingSidecarIndexEntries ?? 0) > 0 ? "warning" : "success", `${(maintenanceReport?.duplicateIndexEntries ?? 0) + (maintenanceReport?.missingSidecarIndexEntries ?? 0)} issue(s)`), escapeHtml2("Deduplicates message/swipe history rows and removes rows that point at missing sidecar snapshots."), `<button class="ltracker-button" type="button" data-action="repair-snapshot-index">Repair Snapshot Index</button>`)}
            ${card("Preset render locks", statusTone((maintenanceReport?.snapshotsWithoutPresetLocks ?? 0) + (maintenanceReport?.snapshotsWithIncompletePresetLocks ?? 0) > 0 ? "warning" : "success", `${(maintenanceReport?.snapshotsWithoutPresetLocks ?? 0) + (maintenanceReport?.snapshotsWithIncompletePresetLocks ?? 0)} issue(s)`), escapeHtml2("Repairs legacy snapshot render locks only when the original installed preset can be matched by id or name/version."), `<button class="ltracker-button" type="button" data-action="repair-preset-render-locks">Repair Preset Render Locks</button>`)}
            ${card("Embedded tags", statusTone((maintenanceReport?.brokenEmbeddedTags ?? 0) > 0 ? "warning" : "success", `${maintenanceReport?.brokenEmbeddedTags ?? 0} broken`), escapeHtml2("Removes only complete LTracker-owned embedded tags whose JSON is broken. Malformed marker fragments are reported for manual review."), `<button class="ltracker-button" type="button" data-action="clean-broken-embedded-tags">Clean Broken Embedded Tags</button>`)}
          </div>
          <div class="ltracker-toolbar" style="margin-top: 12px;">
          ...
          <p class="ltracker-note">${escapeHtml2("Repairs are intentionally conservative. LTracker never silently rebinds old trackers to the current active preset, and orphan cleanup reindexes discoverable sidecars instead of deleting unknown storage.")}</p>
          <div class="ltracker-grid ltracker-details">
          ...
          ${maintenanceReport?.limitationNotes.length ? `<details class="ltracker-details"><summary>Known scan limits</summary><ul class="ltracker-list-compact">${maintenanceReport.limitationNotes.map((note) => `<li>${escapeHtml2(note)}</li>`).join("")}</ul></details>` : ""}
        </section>
        ` : ""}
